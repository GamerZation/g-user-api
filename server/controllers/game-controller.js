...
exports.delete_child = function(req, res) {
  ...
  validate_child(child_type, child_id)
  .then(child => {
    return Game.deleteChild(game_id,child_id,child_type);
  })
  .then(game_model => {
    res.send({ message: 'Child deleted successfully ', game_model })
  })
  .catch(e => {
    res.status(400).send(e);
  })
...
exports.list_child_games = function(req, res) {
  var child_type = req.params.child_type;
  var child_id   = req.params.child_id;
  validate_child(child_type, child_id)
  .then(child => {
    return Game.listChildGames(child_id,child_type);
  })
  .then(games => {
    res.send({ games })
  })
